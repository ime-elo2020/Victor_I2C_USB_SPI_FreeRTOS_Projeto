...
const uint32_t ulValueToSend = 16011; //Alterei a mensagem para meu número de matricula
	...
	for( ;; )
	{
		...
		//O LED irá acender quando o sinal estiver sendo enviado
			  STM_EVAL_LEDOn(LED3);
		//Quando o sinal é enviado, o LED 4 é desligado, representando que está aguardando a mensagem
...
uint32_t ulReceivedValue;
	...
	for( ;; )
	{
		...
		for (j = 0; j < 1000000; j++); //delay para ver melhor a mudança dos LEDs
		...
		{
			//O LED 4 confirma que a mensagem recebida está de acordo com o esperado
			  STM_EVAL_LEDOn(LED4);
		...
		else //se o valor recebido não estiver certo, o LED 4 fica desligado
		{
			STM_EVAL_LEDOff(LED4);
		}
	}
